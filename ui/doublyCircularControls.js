// ui/doublyCircularControls.js
// Wires inputs + buttons to DoublyCircularLinkedListModel and DCLLAnimator.

(function () {
  const initInput = document.getElementById("dcll-elements-input");
  const initBtn = document.getElementById("dcll-init-btn");
  const clearBtn = document.getElementById("dcll-clear-btn");
  const initError = document.getElementById("dcll-init-error");

  const indexInput = document.getElementById("dcll-index-input");
  const valueInput = document.getElementById("dcll-value-input");
  const targetValueInput = document.getElementById("dcll-target-value-input");
  const opButtons = document.querySelectorAll(".dcll-op-btn");
  const opError = document.getElementById("dcll-op-error");

  if (
    !initBtn ||
    !window.DoublyCircularLinkedListModel ||
    !window.DCLLAnimator
  ) {
    return; // not on this page
  }

  const list = new window.DoublyCircularLinkedListModel();
  ...
  function runStepsResult(result) {
    ...
    if (result.ok === false) {
      if (result.steps && result.steps.length > 0) {
        window.DCLLAnimator.playSteps(result.steps);
      }
      return;
    }
    window.DCLLAnimator.playSteps(result.steps || []);
  ...
  clearBtn.addEventListener("click", () => {
    ...
    list.tail = null;
    list.length = 0;
    window.DCLLAnimator.renderState(list.snapshot());
  });
      ...
      try {
        switch (op) {
          ...
          case "traverse-forward":
            result = list.traverseForwardOnceFromHead();
            break;

          case "traverse-backward":
            result = list.traverseBackwardOnceFromTail();
            break;
