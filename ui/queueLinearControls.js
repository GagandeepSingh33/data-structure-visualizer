// ui/queueLinearControls.js
// Wires inputs + buttons to LinearQueueModel and QueueAnimatorLinear.
...
(function () {
  ...
  if (!initBtn || !window.LinearQueueModel || !window.QueueAnimatorLinear) {
    return; // not on this page
  }

  const queue = new window.LinearQueueModel();
  ...
  function runStepsResult(result) {
    ...
    if (steps.length > 0) {
      window.QueueAnimatorLinear.playSteps(steps);
    } else {
      window.QueueAnimatorLinear.renderState(queue.snapshot());
    }
  }
      ...
      try {
        switch (op) {
          case "enqueue":
            if (!valueVal) {
              opError.textContent =
                "Provide a value to enqueue into the queue.";
              return;
            }
          ...
          case "search-value":
            if (!valueVal) {
              opError.textContent =
                "Provide a value to search in the queue.";
              return;
            }
