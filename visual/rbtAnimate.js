// visual/rbtAnimate.js
// Draws red-black trees and highlights colors, rotations, and violations.

const RBTAnimation = (function () {
  const state = {
    model: null,
    treeContainer: null,
    explanationBox: null,
  ...
  function renderTree(snapshot, highlight) {
    ...
    const { storage, colors, size } = snapshot;
    ...
    for (let i = 0; i < size; i++) {
      ...
      // color styling via CSS classes
      if (colors && colors[i] === "RED") node.classList.add("rbt-red");
      else node.classList.add("rbt-black");
      ...
      if (highlight) {
        ...
        if (highlight.recolor && highlight.recolor.includes(i)) {
          node.classList.add("recolor");
        }
  ...
  function renderStep(step) {
    ...
    if (
      ...
      highlight.recolor = step.indices || [];
  ...
  function togglePause(onDone) {
    ...
    if (!state.isPaused) scheduleNext(onDone);
    else clearTimer();
  ...
  function setSpeed(factor) {
    ...
    if (!state.isPaused && isRunning()) scheduleNext();
  }

  function renderAll(snapshot, explanation) {
    renderTree(snapshot, { active: [], shifted: [], recolor: [], invalid: [] });
    if (explanation != null) {
...
window.RBTAnimation = RBTAnimation;
